...
# Configuration
# -------------------------------------------------------------------------
REPO_URL="https://github.com/mateuszstoch/DRCP"

if [[ "$VERSION" == "latest" ]]; then
    DOWNLOAD_URL="$REPO_URL/releases/latest/download/drcp-$target.$ext"
else
    DOWNLOAD_URL="$REPO_URL/releases/download/$VERSION/drcp-$target.$ext"
fi

install_env=DRCP_INSTALL
bin_env=\$$install_env/bin

install_dir=${DRCP_INSTALL:-"$HOME/.drcp"}
...
# Installation
# -------------------------------------------------------------------------
info_bold "Installing drcp ($VERSION) for $target..."
...
if ! curl --fail --location --progress-bar --output "drcp_download.$ext" "$DOWNLOAD_URL"; then
    echo
...
info "Extracting..."
if [[ "$ext" == "zip" ]]; then
    unzip -q -o -j "drcp_download.$ext" -d "$bin_dir" || error "Failed to unzip archive"
else
    tar -xzf "drcp_download.$ext" -C "$bin_dir" || error "Failed to extract tar archive"
fi

# Cleanup
rm -f "drcp_download.$ext"
...
success "drcp ($VERSION) was installed successfully to $Bold_Green$(tildify "$exe")"
